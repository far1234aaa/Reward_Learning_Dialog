...
if [ ${REW_MODEL_EXP} == '0' ]; then
  ...
  sudo cp -r "./damd_multiwoz/Exp${REW_MODEL_EXP}data/." "./damd_multiwoz/Exp${EXP_IDX}data/"
  for seed in "${ALL_SEEDS[@]}"; do
    mkdir -p "./experiments/Exp${EXP_IDX}/all_sd${seed}/"
    sudo cp -r "./experiments/Exp${REW_MODEL_EXP}/all_sd${seed}/reward_model" "./experiments/Exp${EXP_IDX}/all_sd${seed}/"
  done
  wait
fi
